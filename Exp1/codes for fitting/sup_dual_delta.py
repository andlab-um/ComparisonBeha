...
parameters {
  ...
  real<lower=0,upper=1> pos_learning_rate[subjects];
  real<lower=0,upper=1> neg_learning_rate[subjects];
}
...
model {
  ...
  for (s in 1:subjects) {
    ...
    if (action[s,t] != 0) {
      ...
      action[s,t] ~ categorical_logit( (beta_b[s] * v + eb + pb ) ); // compute action probabilities
      ...
      if (pe>0) {
        v[action[s,t]]   = v[action[s,t]] + pos_learning_rate[s] * pe;             // value/mu updating (learning)
      }
      else {
        v[action[s,t]]   = v[action[s,t]] + neg_learning_rate[s] * pe;             // value/mu updating (learning)
      }  
