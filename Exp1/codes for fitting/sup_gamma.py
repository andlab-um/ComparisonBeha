...
parameters {
  ...
  real<lower=0,upper=5> gamma_b[subjects];
}
...
model {
  ...
  for (s in 1:subjects) {
    ...
    vector[4] re;  // random exploration
    vector[4] min_sig;
    ...
    if (action[s,t] != 0) {
      ...
      pb = rep_vector(0.0, 4);
      re = gamma_b[s] * v / (sig[1]+sig[2]+sig[3]+sig[4]);
      ...
      action[s,t] ~ categorical_logit( (beta_b[s] * v + eb + pb + re) ); // compute action probabilities
