...
parameters {
  ...
  real<lower=0,upper=3> comparision_level[subjects];
}
...
model {
  ...
  for (s in 1:subjects) {
    ...
    if (action[s,t] != 0) {
      ...
      pb = rep_vector(0.0, 4);
      compare = rep_vector(0,4);
      if (t>1) {
        if (action[s,t-1] !=0) {
          pb[action[s,t-1]] = persev[s];
          compare[action[s,t-1]] = comparision[s,t];
        } 
      }
      
      happy = v + comparision_level[s] * compare;

      action[s,t] ~ categorical_logit( (beta[s] * happy + eb + pb) ); // compute action probabilities
...
df.to_csv('nhb_competition_action.csv')
