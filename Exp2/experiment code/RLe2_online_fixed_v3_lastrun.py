...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/wututu/Library/CloudStorage/OneDrive-UniversityofMacau/RL_E2_v3/RLe2_online_fixed_v3_lastrun.py',
...
notconfi = visual.ImageStim(
    win=win,
    name='notconfi', 
    image='pic/randomguessingpng.png', mask=None, anchor='center',
    ori=0.0, pos=(-0.7, -0.01), size=(0.229, 0.2),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=-4.0)
...
for thisTrial in trials:
    ...
    import random
    expect_which = random.choice([0,1,2,3])
    ...
    docs.setText('● ● ●')
    wait_playerB.setImage('pic/waitplayerB.png')
    # keep track of which components have finished
    choose_resultComponents = [black, red_2, green_2, yellow_2, blue_2, fixation_2, docs, wait_playerB]
    ...
    while continueRoutine:
        ...
        # *wait_playerB* updates
        if wait_playerB.status == NOT_STARTED and tThisFlip >= 0.3-frameTolerance:
            # keep track of start time/frame for later
            wait_playerB.frameNStart = frameN  # exact frame index
            wait_playerB.tStart = t  # local t and not account for scr refresh
            wait_playerB.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(wait_playerB, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'wait_playerB.started')
            wait_playerB.setAutoDraw(True)
        if wait_playerB.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > wait_playerB.tStartRefresh + z-frameTolerance:
                # keep track of stop time/frame for later
                wait_playerB.tStop = t  # not accounting for scr refresh
                wait_playerB.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'wait_playerB.stopped')
                wait_playerB.setAutoDraw(False)
    ...
    if (choose_bandit.keys == 'r' or choose_bandit.keys == 'f' or choose_bandit.keys == 'i' or choose_bandit.keys == 'j') and(expect_which==2 or expect_which==3):
        continueRoutine = True
    else:
        continueRoutine = False
    slider.reset()
    ...
    routineForceEnded = False
    # update component parameters for each repeat
    # Run 'Begin Routine' code from expect_self_code
    ...
    # keep track of which components have finished
    expectComponents = [expect_self_black, expect_self, expect_self_red, expect_self_green, expect_self_yellow, expect_self_blue, tijiao, expectself, tijiao_click]
    ...
    for thisComponent in expectComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # Run 'End Routine' code from expect_self_code
    thisExp.addData('value_expect_self',expectself.text)
    ...
    black_x_random= random.choice([-0.6,-0.2,0.2,0.6])
    #
    #thisExp.addData('value_expect_random',expect_random_value.text)
    thisExp.addData('random_whicharm', black_x_random)
    thisExp.addData('whether_ask', expect_which)
    ...
    gotValidClick = False  # until a click is received
    # keep track of which components have finished
    expect_randomComponents = [jiantou_random, expect_red, expect_green, expect_yellow, expect_blue, expect_random_2, expect_random_3, tijiao_random_2, tijiao_random]
    ...
    if (choose_bandit.keys == 'r' or choose_bandit.keys == 'f' or choose_bandit.keys == 'i' or choose_bandit.keys == 'j')and(expect_which==1 or expect_which==0) :#and(expect_which==0)
        continueRoutine = True
    else:
        continueRoutine = False
