...
parameters {
  real beta_a_1[subjects]; 
  real phi_a_1[subjects];
  real persev_a_1[subjects];

  real beta_a_0[subjects]; 
  real phi_a_0[subjects];
  real persev_a_0[subjects];
...
model {
  ...
  for (s in 1:subjects) {
    ...
    beta_a_1[s]     ~ normal(0, 0.1);          
    phi_a_1[s]      ~ normal(0, 0.1);  
    persev_a_1[s]   ~ normal(0, 1);
    
    beta_a_0[s]     ~ normal(0, 0.1);          
    phi_a_0[s]      ~ normal(0, 0.1);  
    persev_a_0[s]   ~ normal(0, 1);
    ...
    if (action[s,t] != 0) {
      if (comparision[s,t]>0) {
        ...
        eb = (phi_a_1[s]*comparision[s,t]+phi_b[s]) * relative_sig;
        pb = rep_vector(0.0, 4);
            
        if (t>1) {
            if (action[s,t-1] !=0) {
            pb[action[s,t-1]] = persev_a_1[s]*comparision[s,t]+persev_b[s];
            } 
        }
        beta_t = beta_a_1[s]*comparision[s,t]+beta_b[s];
        if (beta_t <0){
        beta_t = 0;
        }
        ...
        if (t>1) {
            if (action[s,t-1] !=0) {
            pb[action[s,t-1]] = persev_a_0[s]*comparision[s,t]+persev_b[s];
            } 
        }
        beta_t = beta_a_0[s]*comparision[s,t]+beta_b[s];
        if (beta_t <0){
        beta_t = 0;
        }
        action[s,t] ~ categorical_logit( beta_t * v + eb + pb ); // compute action probabilities
...
for i in range(12):
    if i != 11:
        bandit_data = {"subjects":5,
                    "trials":len(action[0]),
                    "action":action[i*5:i*5+5],
                    "reward":reward[i*5:i*5+5],
                    "comparision":comparision[i*5:i*5+5]}
        ...
        fitting_result_file_name = './model_glm2_vep_'+str(i)+'_exp2.csv'
        df.to_csv(fitting_result_file_name)
    else:
        bandit_data = {"subjects":7,
                    "trials":len(action[0]),
                    "action":action[i*5:i*5+7],
                    "reward":reward[i*5:i*5+7],
                    "comparision":comparision[i*5:i*5+7]}
        sm = pystan.StanModel(model_code=bandit_model)
        fit = sm.sampling(data=bandit_data,iter=4000,chains=4,warmup=2000)
        summary_dict = fit.summary()
        df = pd.DataFrame(summary_dict['summary'],columns=summary_dict['summary_colnames'],index=summary_dict['summary_rownames'])
