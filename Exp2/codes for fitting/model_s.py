...
parameters {
  real<lower=0> comparison_level[subjects];
...
model {
  ...
  for (s in 1:subjects) {
    ...
    comparison_level[s]      ~ normal(0, 0.1);  
    ...
    if (action[s,t] != 0) {
      ...
      action[s,t] ~ categorical_logit( (beta_b[s] * (v+comparison_level[s]*compare) + eb + pb ) ); // compute action probabilities
...
for i in range(6):
    if i != 5:
        ...
        fitting_result_file_name = './model_s_'+str(i)+'_exp2.csv'
