...
parameters {
  real beta_a[subjects]; 
  real phi_a[subjects];
  real persev_a[subjects];
...
model {
  ...
  for (s in 1:subjects) {
    ...
    beta_a[s]     ~ normal(0, 0.1);          
    phi_a[s]      ~ normal(0, 0.1);  
    persev_a[s]   ~ normal(0, 1);
    ...
    if (action[s,t] != 0) {
      ...
      beta_t = beta_a[s]*comparision[s,t]+beta_b[s];
      if (beta_t <0){
      beta_t = 0;
      }
...
for i in range(6):
    if i != 5:
        ...
        fitting_result_file_name = './model_glm_vep_'+str(i)+'_exp2.csv'
...
for i in range(12):
    if i != 11:
        ...
        summary_dict = fit.summary()
        df = pd.DataFrame(summary_dict['summary'],columns=summary_dict['summary_colnames'],index=summary_dict['summary_rownames'])
        fitting_result_file_name = './model_glm2__'+str(i)+'_exp2.csv'
        df.to_csv(fitting_result_file_name)
    else:
